...
module Akami
  class WSSE
    class Signature
      ...
      def timestamp_id
        @timestamp_id ||= "Timestamp-2".freeze
      end
      ...
      def signed_info
        {
          "SignedInfo" => {
            ...
            "SignatureMethod/" => nil,
            "Reference" => [
              signed_info_transforms.merge(signed_info_digest_method).merge({ "DigestValue" => body_digest, :@URI =>  "##{body_id}"}),
              signed_info_transforms.merge(signed_info_digest_method).merge({ "DigestValue" => timestamp_digest, :@URI =>  "##{timestamp_id}"}),
      ...
      def the_signature
        ...
        signed_info1 = at_xpath(@document, "//Envelope/Header/Security/Signature/SignedInfo")
        signed_info1 = signed_info1 ? canonicalize(signed_info1) : ""

        
        signature = certs.private_key.sign(OpenSSL::Digest::SHA1.new, signed_info1)
